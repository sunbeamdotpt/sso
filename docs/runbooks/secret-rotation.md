...
1. Generate new values in Vault/OpenBao at the paths referenced by `vault-secrets.yaml`.
2. Update the secret values:
   - `secret/kratos`
   - `secret/kratos-ui`
   - `secret/hydra`
3. Vault Secrets Operator (VSO) will update the K8s Secrets within the configured `refreshAfter` window (default 1h).
...
Database credentials are managed by Vault DynamicSecret (`kratos-db-creds`, `hydra-db-creds`).
...
- Rotating cookie/session secrets invalidates existing sessions; users must log in again.
- Coordinate rotations outside peak hours and communicate via `#incidents` if user-facing.
