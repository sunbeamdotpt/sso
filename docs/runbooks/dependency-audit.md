...
- **Backend server**: Rust crate in `api/`, built on `sunbeam-g2v`. Audited with `cargo audit` (requires `cargo-audit` installed).
...
2. For transitive issues, run `cargo update` to pull the latest compatible versions, or pin the transitive crate in `Cargo.toml`.
...
Always commit `deno.lock` and `api/Cargo.lock`. After updating dependencies, run:
