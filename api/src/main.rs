...
async fn security_headers_layer(
    ...
    headers.insert(
        ...
        HeaderValue::from_static(
            "default-src 'self'; script-src 'self' 'sha256-p9zWcqCFG7c70MXUvDmtqVRotRuoLSD5Dpv46xu+n/o='; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' https: https://fonts.gstatic.com; connect-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self';", 
        ),
    );
    response
}
